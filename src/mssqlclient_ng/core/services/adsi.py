...
class AdsiService:
    ...
    def _get_ldap_server_assembly(self) -> tuple[str, str]:
        ...
        Source: https://github.com/blackarrowsec/redteam-research/pull/13
        Raw:    https://raw.githubusercontent.com/blackarrowsec/redteam-research/2b1cdcb67a490880ce0b0714983cbd707cb8b770/MSSQL%20linked%20servers%20-%20ADSI/ldapServer.cs

        Returns:
            tuple of (assembly_hex_bytes, assembly_sha512_hash)
        """
        # Embedded as gzip+base64 to keep binary overhead minimal (~17 kB)
        ...
        # Decode and decompress back to original bytes, then derive hash + hex
        decompressed_bytes = common.decode_and_decompress(compressed_base64)
        library_hash, library_hex_bytes = common.convert_dll_to_sql_bytes(
            decompressed_bytes
        )

        return (library_hex_bytes, library_hash)
